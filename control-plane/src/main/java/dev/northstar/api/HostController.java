...
public class HostController {
  ...
  @PostMapping("/hosts/{id}/commands/lease") public ResponseEntity<List<CommandRecord>> lease(@PathVariable UUID id,@RequestParam(defaultValue="10")int limit){if(repo.findHost(id).isEmpty())return ResponseEntity.notFound().build();OffsetDateTime now=OffsetDateTime.now(ZoneOffset.UTC);return ResponseEntity.ok(repo.leaseCommands(id,Math.max(1,Math.min(limit,100)),now,now.plus(COMMAND_LEASE)));}
  @PostMapping("/commands/{id}/ack") public ResponseEntity<CommandRecord> ack(@PathVariable UUID id,@Valid @RequestBody CommandAckRequest q){return repo.acknowledge(id,q.leaseToken(),OffsetDateTime.now(ZoneOffset.UTC)).map(ResponseEntity::ok).orElseGet(()->ResponseEntity.status(409).build());}
