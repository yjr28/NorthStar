...
class NorthStarApiIntegrationTest {
    ...
    private void register(UUID id,String name){http.postForEntity(url("/api/v1/hosts"),Map.of("id",id.toString(),"hostname",name,"agentToken",TOKEN),Map.class);}
    ...
    @Test void hostTelemetryAndIdempotentCommandFlow() {
        ...
        HttpHeaders headers=new HttpHeaders();headers.set("Idempotency-Key","integration-command-1");
