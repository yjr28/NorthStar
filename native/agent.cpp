...
static size_t drain_spool(const std::string &path,const std::string &h,const std::string &p,const std::string &endpoint,const std::string &token){auto lines=northstar::read_spool(path);size_t sent=0;while(sent<lines.size()&&post_ok(h,p,endpoint,lines[sent],token))++sent;if(sent>0)northstar::replace_spool_atomically(path,lines,sent);return sent;}
