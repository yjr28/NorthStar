...
static int http_post(const std::string &host,const std::string &port,const std::string &path,const std::string &body,const std::string &token=""){
...
static bool post_ok(const std::string &h,const std::string &p,const std::string &path,const std::string &body,const std::string &token=""){try{int s=http_post(h,p,path,body,token);return s>=200&&s<300;}catch(const std::exception &e){std::cerr<<"POST "<<path<<" failed: "<<e.what()<<"\n";return false;}}
...
static size_t drain_spool(const std::string &path,const std::string &h,const std::string &p,const std::string &endpoint,const std::string &token){std::ifstream in(path);if(!in)return 0;std::vector<std::string> lines;std::string line;while(std::getline(in,line))if(!line.empty())lines.push_back(line);size_t sent=0;while(sent<lines.size()&&post_ok(h,p,endpoint,lines[sent],token))++sent;if(sent==lines.size()){in.close();std::error_code ec;std::filesystem::remove(path,ec);return sent;}std::ofstream out(path,std::ios::trunc);for(size_t i=sent;i<lines.size();++i)out<<lines[i]<<'\n';return sent;}
